<?php  
	...
	if($id!='')
	{
		...
		// Lấy ảnh Slide List 
		$slide_list = $d->rawQuery("select photo, name$lang, link_photo, id from #_gallery where id_parent = ? and type = ? and val = ? and find_in_set('hienthi',status) order by numb,id desc",array($productList['id'],'san-pham','slide_list'));
	...
	else if($idc!='')
	{
		...
		$danhmucItem = $d->rawQuery("select * from #_product_item where id_cat = ? and type = ? order by numb,id desc",array($idc,$type));
		
		/* Lấy hình Slide & Banner */
		$pro_slide = $d->rawQuery("select name$lang, link_photo, photo, id from #_gallery where id_parent = ? and com='product' and type = ? and val = ? and find_in_set('hienthi',status) order by numb,id desc",array($idc,$type,'slide_cat'));
		$pro_banner = $d->rawQuery("select name$lang, link_photo, photo, id from #_gallery where id_parent = ? and com='product' and type = ? and val = ? and find_in_set('hienthi',status) order by numb,id desc",array($idc,$type,'banner_cat'));
	...
	else if($idi!='')
	{
		...
		$danhmucSub = $d->rawQuery("select * from #_product_sub where id_item = ? and type = ? order by numb,id desc",array($idi,$type));
		
		/* Lấy hình Slide & Banner */
		$pro_slide = $d->rawQuery("select name$lang, link_photo, photo, id from #_gallery where id_parent = ? and com='product' and type = ? and val = ? and find_in_set('hienthi',status) order by numb,id desc",array($idi,$type,'slide_item'));
		$pro_banner = $d->rawQuery("select name$lang, link_photo, photo, id from #_gallery where id_parent = ? and com='product' and type = ? and val = ? and find_in_set('hienthi',status) order by numb,id desc",array($idi,$type,'banner_item'));
	...
	{
		/* Lấy hình Slide & Banner */
		$pro_slide = $d->rawQuery("select name$lang, link_photo, photo, id from #_gallery where id_parent = ? and com='product' and type = ? and val = ? and find_in_set('hienthi',status) order by numb,id desc",array($ids,$type,'slide_sub'));
		$pro_banner = $d->rawQuery("select name$lang, link_photo, photo, id from #_gallery where id_parent = ? and com='product' and type = ? and val = ? and find_in_set('hienthi',status) order by numb,id desc",array($ids,$type,'banner_sub'));
	...
	else
	{
		...
		/* Lấy tất cả sản phẩm */
		$where = "";
		$where = "type = ? and discount > 0 and find_in_set('hienthi',status)";
		$params = array($type);
