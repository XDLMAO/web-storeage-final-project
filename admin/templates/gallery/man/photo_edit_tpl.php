<?php
    ...
    if($kind=="man_list") $dfgallery = ($kind == 'man_list') ? 'gallery_list' : 'gallery';
	if($kind=="man_cat") $dfgallery = ($kind == 'man_cat') ? 'gallery_cat' : 'gallery';
	if($kind=="man_item") $dfgallery = ($kind == 'man_item') ? 'gallery_item' : 'gallery';
	if($kind=="man_sub") $dfgallery = ($kind == 'man_sub') ? 'gallery_sub' : 'gallery';
?>
<!-- Content Header -->
                        ...
                        <div class="upload-file">
                            ...
                            <label class="upload-file-label mb-2" for="file">
                                <div class="upload-file-image rounded mb-3">
                                    <?=$func->getImage(['class' => 'rounded img-upload', 'sizes' => '250x250x3', 'upload' => 'upload/'.$com.'/', 'image' => (!empty($item['photo'])) ? $item['photo'] : '', 'alt' => 'Alt Photo'])?>
                                </div>
                                ...
                                <?php foreach($config['website']['lang'] as $k => $v) { ?>
                                    <div class="tab-pane fade show <?=($k=='vi')?'active':''?>" id="tabs-lang-<?=$k?>" role="tabpanel" aria-labelledby="tabs-lang">
                                        ...
                                        <?php if(isset($config[$com][$type][$dfgallery][$val]['link_photo']) && $config[$com][$type][$dfgallery][$val]['link_photo'] == true) { ?>
                                            <div class="form-group">
                                                <label for=link_photo">Link:</label>
                                                <input type="text" class="form-control text-sm" name="data[link_photo]" id=link_photo" placeholder="Link" value="<?=@$item['link_photo']?>">
                                            </div>
                                        <?php } ?>
                                    </div>
                                <?php } ?>
