<?php
	$linkMan = $linkFilter = "index.php?com=product&act=man_item&type=".$type;
	$linkAdd = "index.php?com=product&act=add_item&type=".$type;
    $linkEdit = "index.php?com=product&act=edit_item&type=".$type;
    $linkDelete = "index.php?com=product&act=delete_item&type=".$type;
    $linkMulti = "index.php?com=product&act=man_photo&kind=man_item&type=".$type;
?>
<!-- Content Header -->
            ...
            <table class="table table-hover">
                <thead>
                    <tr>
                        ...
                        <?php if(isset($config['product'][$type]['gallery_item']) && count($config['product'][$type]['gallery_item']) > 0) { ?>
                    ...
                    <tbody>
                        <?php for($i=0;$i<count($items);$i++) {
                            ...
                            <tr>
                                ...
                                <td class="align-middle">
                                    <input type="number" class="form-control form-control-mini m-auto update-numb" min="0" value="<?=$items[$i]['numb']?>" data-id="<?=$items[$i]['id']?>" data-table="product_item">
                                </td>
                                <?php if(isset($config['product'][$type]['show_images_item']) && $config['product'][$type]['show_images_item'] == true) { ?>
                                    <td class="align-middle">
                                    	<a href="<?=$linkEdit?><?=$linkID?>&id=<?=$items[$i]['id']?>" title="<?=$items[$i]['namevi']?>">
                                            <?=$func->getImage(['class' => 'rounded img-preview', 'sizes' => $config['product'][$type]['thumb_item'], 'upload' => UPLOAD_PRODUCT_L, 'image' => $items[$i]['photo'], 'alt' => $items[$i]['namevi']])?>
                                        </a>
                                    </td>
                                <?php } ?>
                                <td class="align-middle">
                                    ...
                                    <td class="align-middle">
                                        <div class="dropdown">
                                            ...
                                            <div class="dropdown-menu" aria-labelledby="dropdown-gallery">
                                                <?php foreach($config['product'][$type]['gallery_item'] as $key => $value) { ?>
                                ...
                                <?php if(isset($config['product'][$type]['check_item'])) { foreach($config['product'][$type]['check_item'] as $key => $value) { ?>
                                    <td class="align-middle text-center">
                                        <div class="custom-control custom-checkbox my-checkbox">
                                            <input type="checkbox" class="custom-control-input show-checkbox" id="show-checkbox-<?=$key?>-<?=$items[$i]['id']?>" data-table="product_item" data-id="<?=$items[$i]['id']?>" data-attr="<?=$key?>" <?=(in_array($key, $status_array)) ? 'checked' : ''?>>
