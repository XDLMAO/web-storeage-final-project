		                        ...
		                        <div class="tab-content" id="custom-tabs-three-tabContent-lang">
		                            <?php foreach($config['website']['lang'] as $k => $v) { ?>
		                                <div class="tab-pane fade show <?=($k=='vi')?'active':''?>" id="tabs-lang-<?=$k?>" role="tabpanel" aria-labelledby="tabs-lang">
		                                    ...
		                                    <?php } ?>
											<?php if(isset($config['product'][$type]['guarantee']) && $config['product'][$type]['guarantee'] == true) { ?>
		                                        <div class="form-group">
		                                            <label for="guarantee<?=$k?>">Bảo hành (<?=$k?>):</label>
		                                            <textarea class="form-control text-sm <?=(isset($config['product'][$type]['guarantee_cke']) && $config['product'][$type]['guarantee_cke'] == true)?'form-control-ckeditor':''?>" name="data[guarantee<?=$k?>]" id="guarantee<?=$k?>" rows="1" placeholder="Bảo hành (<?=$k?>)"><?=htmlspecialchars_decode((!empty($flash->has('guarantee'.$k))) ? $flash->get('guarantee'.$k) : @$item['guarantee'.$k])?></textarea>
		                                        </div>
		                                    <?php } ?>
		                                </div>
		                            <?php } ?>
